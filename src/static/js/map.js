...
window.__flyerZoom = function (src) {
  let lb = document.getElementById('flyer-zoom');
  if (!lb) {
    lb = document.createElement('div');
    lb.id = 'flyer-zoom';
    lb.style.cssText = 'position:fixed;inset:0;background:rgba(15,10,8,.82);z-index:3000;' +
      'display:flex;align-items:center;justify-content:center;cursor:zoom-out;padding:30px;';
    lb.innerHTML = '<img style="max-width:min(92vw,640px);max-height:92vh;border-radius:12px;' +
      'box-shadow:0 30px 80px -20px rgba(0,0,0,.8);" alt="Flyer ampliado">';
...
function campPopupHTML(c) {
  ...
  const f = c.dia.toLocaleDateString('es-CO', { weekday: 'long', day: 'numeric', month: 'long' });
  const titulo = c.titulo || ('Campaña de donación — ' + z.nombre);
  const flyerImg = c.flyer
    ? '<img class="popup-flyer" src="' + c.flyer + '" alt="Flyer de la campaña" loading="lazy" ' +
      'title="Click para ampliar" onclick="window.__flyerZoom(this.src)">'
    : '';
  return popupHTML('🩸 ' + titulo,
    '📍 ' + z.lugar + ' · ' + z.nombre +
    '<br>📅 ' + f +
    '<br>🩸 Grupo prioritario: <b>' + (c.tipo || 'Todos') + '</b>' +
    '<br>Estado: <b>' + (c.estado || 'desplegada') + '</b>',
    '') + flyerImg;
}
...
function renderCampaignMarkers() {
  ...
  if (isEarthquake) {
    Object.entries(ZONAS).forEach(([key, z]) => {
      ...
      el.textContent = { shelter: '⛺', donation: '🩸', affected: '⚠️' }[z.tipo] || '';
      if (z.tipo === 'affected') {
...
/* Si venimos del registro de campañas ("Ver en el mapa"), vuela a esa zona y
   abre el popup de su campaña. */
function focusZoneFromStorage() {
  let key = null;
  try { key = localStorage.getItem('sang-focus-zona'); } catch (_) { /* sin storage */ }
  if (!key || !map3d || !ZONAS[key]) return;
  try { localStorage.removeItem('sang-focus-zona'); } catch (_) { /* ignora */ }
  const z = ZONAS[key];
  map3d.flyTo({ center: [z.lng, z.lat], zoom: 14.5, pitch: 55, duration: 1800 });
  const marker = campMarkers.find(m => m._zonaKey === key);
  if (marker && marker.getPopup()) setTimeout(() => marker.togglePopup(), 1900);
}
...
function renderMap3DScenario() {
  ...
  hgmEl.innerHTML = '<span class="mk-hgm-ico">🏥</span>';
